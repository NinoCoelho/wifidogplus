...
auth_server_request(t_authresponse *authresponse, const char *request_type, const char *ip, const char *mac, const char *token, unsigned long long int incoming, unsigned long long int outgoing)
{
    ...
    if (!strncasecmp(dev_name, DUMY_HOST_NAME, strlen(DUMY_HOST_NAME) + 1)) {
		debug(LOG_DEBUG, "dev_name_mac: [%s]", mac);
#ifdef __OPENWRT__
        ...
        if (get_hostname(mac, dev_name)) {
            memcpy(dev_name, DUMY_HOST_NAME, strlen(DUMY_HOST_NAME) + 1);
        }
#endif
        debug(LOG_DEBUG, "%s:device name is [%s]",__func__,dev_name);
	...
	snprintf(buf, (sizeof(buf) - 1),
		"GET %s%sstage=%s&dev_name=%s&ip=%s&mac=%s&token=%s&incoming=%llu&outgoing=%llu&gw_id=%s HTTP/1.0\r\n"
	...
	if ((tmp = strstr(buf, "Auth: "))) {
        ...
        debug(LOG_WARNING, "Could not read anything");
        return(AUTH_ERROR);
    }
	else {
		return(AUTH_ERROR);
...
int _connect_auth_server(int level) {
	...
	if (level > num_servers) {
        ...
        * cjpthree@126.com 2015.5.14 add notify get auth address thread start work
		 */
		debug(LOG_DEBUG, "Can not find a auth server can work, notify get auth address thread start work again");
#if OPEN_THREAD_GETADDRESS
		pthread_cond_signal(&get_address_thread_cond);
#endif
		return (-1);
	}
	...
	debug(LOG_DEBUG, "Level %d: Resolving auth server [%s:%d]", level, hostname, auth_server->authserv_http_port);
	...
	if (!h_addr) {
		...
		if (h_addr) {
            ...
            */
            while (!is_network_welldone()) {
                debug(LOG_ERR, "network need to restart");
                network_restart();
            }
#endif
...
/* Helper function called by connect_app_server() to do the actual work including recursion
 * DO NOT CALL DIRECTLY
 @param level recursion level indicator must be 0 when not called by _connect_app_server()
 */
int _connect_app_server(int level) {
	s_config *config = config_get_config();
	t_app_serv *app_server = NULL;
	struct in_addr *h_addr;
	int num_servers = 0;
	char * hostname = NULL;
	char * popular_servers[] = {
	 ...
	 * Let's calculate the number of servers we have
	 */
	for (app_server = config->app_servers; app_server; app_server = app_server->next) {
		num_servers++;
	}
	debug(LOG_DEBUG, "Level %d: Calculated %d app servers in list", level, num_servers);
	 ...
	 * Let's resolve the hostname of the top server to an IP address
	 */
	app_server = config->app_servers;
	hostname = app_server->appserv_hostname;
	debug(LOG_DEBUG, "Level %d: Resolving app server [%s:%d]", level, hostname, app_server->appserv_http_port);
	h_addr = wd_gethostbyname(hostname);
	if (!h_addr) {
		/*
		 * DNS resolving it failed
		 ...
		 * Can we resolve any of the popular servers ?
		 */
		debug(LOG_DEBUG, "Level %d: Resolving app server [%s] failed", level, hostname);
		...
		if (h_addr) {
			 ...
			 * Yes
			 *
			 * The app server's DNS server is probably dead. Try the next app server
			 */
			debug(LOG_DEBUG, "Level %d: Marking app server [%s] as bad and trying next if possible", level, hostname);
			if (app_server->last_ip) {
				free(app_server->last_ip);
				app_server->last_ip = NULL;
			}
			mark_app_server_bad(app_server); /* move this sever to the last of the sever list */
			return _connect_app_server(level);
		}
		else {
			/*
			 * No
...
#endif
			mark_offline();
			debug(LOG_DEBUG, "Level %d: Failed to resolve app server and all popular servers. "
					"The internet connection is probably down", level);
			return(-1);
		}
	}
		 ...
		 */
		ip = safe_strdup(inet_ntoa(*h_addr));
		debug(LOG_DEBUG, "Level %d: Resolving app server [%s] succeeded = [%s]", level, hostname, ip);

		if (!app_server->last_ip || strcmp(app_server->last_ip, ip) != 0) {
			/*
			 * But the IP address is different from the last one we knew
			 * Update it
			 */
			debug(LOG_DEBUG, "Level %d: Updating last_ip IP of server [%s] to [%s]", level, hostname, ip);
			if (app_server->last_ip) free(app_server->last_ip);
			app_server->last_ip = ip;
		 ...
		 * Connect to it
		 */
		debug(LOG_DEBUG, "Level %d: Connecting to app server %s:%d", level, hostname, app_server->appserv_http_port);
		their_addr.sin_family = AF_INET;
		their_addr.sin_port = htons(app_server->appserv_http_port);
		their_addr.sin_addr = *h_addr;
		memset(&(their_addr.sin_zero), '\0', sizeof(their_addr.sin_zero));
		free (h_addr);
		...
		if (connect(sockfd, (struct sockaddr *)&their_addr, sizeof(struct sockaddr)) == -1) {
			 ...
			 * Mark the server as bad and try the next one
			 */
			debug(LOG_DEBUG, "Level %d: Failed to connect to app server %s:%d (%s). Marking it as bad and trying next if possible",
			level, hostname, app_server->appserv_http_port, strerror(errno));
			close(sockfd);
			mark_app_server_bad(app_server);
			return _connect_app_server(level); /* Yay recursion! */
		}
		else {
			/*
			 * We have successfully connected
			 */
			debug(LOG_DEBUG, "Level %d: Successfully connected to app server %s:%d", level, hostname, app_server->appserv_http_port);
			return sockfd;
		}
	}
}
...
/* Tries really hard to connect to an app server. Returns a file descriptor, -1 on error
 * Part of the camouflage a app server
 * cjpthree@126.com 2015.5.8
 */
int connect_app_server() {
	int sockfd;

	//LOCK_CONFIG(); xxxc
	sockfd = _connect_app_server(0);
	//UNLOCK_CONFIG(); xxxc

	if (sockfd == -1) {
		debug(LOG_ERR, "Failed to connect to any of the app servers");
		mark_appserv_offline();
	}
	else {
		debug(LOG_DEBUG, "Connected to app server");
		mark_appserv_online();
	}
	return (sockfd);
}
